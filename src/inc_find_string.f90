...
!--------------------------------------------------------------
  SUBROUTINE find_string(this, error_out)
    !--------------------------------------------------------------
    ! This subroutine stores the line of the file which starts
    ! by the string.
    !--------------------------------------------------------------
    IMPLICIT NONE
    ...
    !-- First we check that the input file exists --
    inquire(FILE = this%file, EXIST = file_exists)
    IF (.NOT.(file_exists)) THEN
       WRITE(*,*) '*** Input error: the file ', trim(this%file), &
            ' does not exist. ***'
       error_out = 3
       GOTO 1000 !-- End of subroutine
    ENDIF
