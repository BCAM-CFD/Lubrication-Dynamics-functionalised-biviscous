...
!--------------------------------------------------
SUBROUTINE vel_iterative_biviscous(this, time, error_out)
!--------------------------------------------------
  ...
  CALL vel_implicit_lub_biviscous(this, N_sweep, error_out)
  ...
  DO WHILE (N_sweep1 >= 1 .AND. error_decrease < this%sweep_tol)
     ...
     CALL vel_implicit_lub_biviscous(this, N_sweep1, error_out)
     IF (error_out .NE. 0) THEN
        GOTO 1000 !-- End of subroutine --
     ENDIF
  ...
  ELSE
     ...
     DO  WHILE (N_sweep2 <= this%N_sweep_max .AND. &
        ...
        CALL vel_implicit_lub_biviscous(this, N_sweep2, error_out)
        IF (error_out .NE. 0) THEN
           GOTO 1000 !-- End of subroutine --
        ENDIF
...
END SUBROUTINE vel_iterative_biviscous
